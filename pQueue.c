...
void addPQueue_LI(pQueue **queue, htNode * val, int direction, int parental_node)//add node without sorting - for simple LIFO queue
{
	//If the queue is full we don't have to add the specified value.
	//We output an error message to the console and return.
	if((*queue)->size == MAX_SZ)
	...
	pQueueNode *aux = (pQueueNode *)malloc(sizeof(pQueueNode));
	aux->val = val;
	aux->val->direction = direction;
	aux ->val ->parental_node = parental_node;
	...
	{
		//we make aux last node in queue
...
htNode * getPQueue_FO(pQueue **queue)//FO implementation
{
	htNode * returnValue = NULL;
